    ...
    <div class="container mx-auto flex items-center justify-between">
        ...
        <div class="flex items-center space-x-6">
            ...
            <div class="hidden md:flex space-x-6">
                ...
                {{-- NEW: Link to customer's order history --}}
                @auth
                    <a href="{{ route('customer.orders.index') }}" class="hover:text-green-400 transition">My Orders</a>
                @endauth
            </div>
        </div>
        ...
        <div class="flex items-center space-x-4">
            ...
            {{-- New: Conditional Links for Admin and Vendor --}}
            @auth
                @if(auth()->user()->role === 'admin')
                    <a href="{{ route('admin.dashboard') }}" class="hover:text-green-400 transition font-medium">Admin Panel</a>
                @endif
                @if(auth()->user()->role === 'vendor')
                    <a href="{{ route('vendor.dashboard') }}" class="hover:text-green-400 transition font-medium">Vendor Panel</a>
                @endif
            @endauth

            {{-- Existing Auth Links and Icons --}}
            @auth
                <a href="{{ route('profile.edit') }}" class="hover:text-green-400 transition" aria-label="Profile">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <a href="{{ route('cart.index') }}" class="relative hover:text-green-400 transition" aria-label="Cart">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            {{-- Logout Link (moved to be next to Profile link) --}}
            @auth
                <form method="POST" action="{{ route('logout') }}" class="inline-block">
                    @csrf
                    <button type="submit" class="hover:text-red-400 transition p-1" aria-label="Log out">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
