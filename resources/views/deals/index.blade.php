            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                @forelse ($deals as $deal)
                    <div x-data="{}" class="bg-white rounded-lg shadow-md overflow-hidden flex flex-col items-center p-4 relative">
                        ...
                        {{-- NEW UPGRADE: We are now wrapping the product info in a link --}}
                        <a href="{{ route('products.show', $deal->id) }}" class="flex flex-col items-center">
                            <img src="{{ asset('storage/' . $deal->image) }}" alt="{{ $deal->name }}" class="w-36 h-36 object-cover rounded-md mb-3">
                            ...
                            <div class="flex items-center justify-center mb-3">
                                {{-- NEW: Display original price with a strikethrough --}}
                                <span class="text-gray-400 text-sm line-through mr-2">${{ number_format($deal->price, 2) }}</span>
                                {{-- NEW: Calculate and display the discounted price --}}
                                @php
                                    $discountedPrice = $deal->price - ($deal->price * $deal->discount_percentage / 100);
                                @endphp
                                <span class="text-green-600 font-bold text-base">${{ number_format($discountedPrice, 2) }}</span>
                            </div>
                        </a>
                        
                        {{-- The Add to Cart button remains outside the link --}}
                        <button @click="
                            fetch('{{ route('cart.add') }}', {
                                ...
                                body: JSON.stringify({
                                    ...
                                    // NEW: Pass the discounted price
                                    price: parseFloat({{ Js::from($discountedPrice) }}),
                            ...
                            .catch(error => {
                                ...
                                alert('Error communicating with server. Failed to add item to cart.');
                            });
                ...
                @empty
                    <div class="col-span-full text-center py-10">
                        <p class="text-gray-600 text-lg">No deals are currently available.</p>
                    </div>
                @endforelse
            </div>
